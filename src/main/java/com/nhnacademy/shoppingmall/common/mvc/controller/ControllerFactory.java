...
//웹 어플리케이션에서 사용되는 컨트롤러 객체를 생성하고 관리
//주로 웹 요청을받아 적절한 컨터롤러를 찾아 실행하는데 사용 -> 컨트롤러 객체를 효율적으로 관리
                ...
                for (Class<?> controllerClass : c) {
                    ...
                    if(controllerClass.isAnnotationPresent(RequestMapping.class)){
                        ...
                        for (String path : requestMapping.value()) {
                            String key = getKey(requestMapping.method().toString(), path);
                            beanMap.put(key, controller);
                            log.debug("Added to beanMap with key " + key + " - " + controller);
                        }
                    }
                }
            } catch (InstantiationException | IllegalAccessException | InvocationTargetException | NoSuchMethodException e) {
                throw new RuntimeException("Error initializing controllers", e);
        }
